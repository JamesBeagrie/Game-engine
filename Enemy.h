...
public:
	...
	void draw(unsigned char*& bits, int frameCount, int bottom, int right, float*& zBuffer, float playerRotationX, float playerRotationY, vec3 playerPos, float FOV) {
		triangleArr->draw(bits, frameCount, bottom, right, zBuffer, playerRotationX, playerRotationY, playerPos, FOV);
	}
	...
	void checkBulletCollision(vec3* intersectionPoint, float* nearestDistance, bool* foundCollisionAtAll, vec3 bPos, vec3 bVel, vec3 bulletSize, float damage) {

		bool foundCollision = false;
		triangleArr->checkIfHitsPlane(intersectionPoint, nearestDistance, &foundCollision, bPos, bVel, bulletSize);
	...
	void changeHealth(float offset) {
		float tempHealth = HP + offset;
