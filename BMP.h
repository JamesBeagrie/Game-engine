...
struct BMP {
    ...
    bool get_pixel(uint32_t x, uint32_t y, uint8_t& r, uint8_t& g, uint8_t& b, uint8_t& a) const {
        // Ensure the coordinates are within the image boundaries
        if (x >= static_cast<uint32_t>(bmp_info_header.width) || y >= static_cast<uint32_t>(bmp_info_header.height)) {
            return false; // Coordinates are out of bounds
        }

        // Calculate the offset to the pixel data
        uint32_t channels = bmp_info_header.bit_count / 8;
        uint32_t offset = y * row_stride + x * channels;

        // Extract color values (assuming BGRA format)
        b = data[offset];
        g = data[offset + 1];
        r = data[offset + 2];
        ...
        if (channels == 4) {
            ...
            a = 255; // If no alpha channel, assume fully opaque
        }
